...
<h1 class="font-bold text-slate-600 text-3xl">Kategori</h1>
...
<div class="garis mt-10 mb-3">
    <div class="bg-slate-100 pr-3 text-lg font-medium text-slate-600">Kategori</div>
</div>

<a href="{{ route('buat-kategori') }}" class="ml-auto shadow flex mb-5 mt-3 justify-center items-center text-sm py-3 font-semibold rounded text-white bg-green-500">
    <i data-feather="plus" width="24px" height="24px" class="mr-1"></i> TAMBAH KATEGORI
</a>
...
<div class="relative ">
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400 data-table overflow-x-auto shadow-md">
        <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
            <tr>
                <th class="px-6 py-3" scope="col">Nama Kategori</th>
                <th class="py-3" scope="col">Biaya Perjam</th>
                <th class="py-3" scope="col">Upah Makan</th>
...
$(document).ready(function() {
    $('.data-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: "{{ route('table_kategori') }}",
        columns: [
            { data: 'nama_kategori', name: 'nama_kategori' },
            { data: 'biaya', name: 'biaya' },
            { data: 'upah_makan', name: 'upah_makan' },
